...
struct monitored_entry {
    pid_t pid;
    char container_id[64];           // keep container name/id
    unsigned long soft_limit_bytes;
    unsigned long hard_limit_bytes;
    bool soft_warned;                // have we already emitted soft-limit warning?
 ...
 * ============================================================== */
static LIST_HEAD(monitored_list);  // head of linked list
static DEFINE_MUTEX(monitored_lock); // protects insert/remove/iteration
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        printk(KERN_INFO "[container_monitor] PID %d RSS %ld bytes\n", entry->pid, rss);
